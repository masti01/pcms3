...
class Demography():
    ...
    def _demo_data(self,sid):
        ...
        # drowR = re.compile(r'cmid\"\>[\d\.]*?(?P<year>\d{4})\<\/td\>\n.*?ind\"\>(?P<population>\d*)<\/td\>\n.*?ind\"\>Census')
        # drowR = re.compile(
        #    r'<tr>\n\s*?<td class=\"cmid\">.*?(?P<year>\d{4})</td>\n\s*?<td class=\"ind\">.*?(?P<population>\d*?)</td>\n\s*?<td class=\"ind\">Census.*?</td>\n</tr>')
    ...
    def demo_template(self):
        ...
        for year, pop in self.demo_data:
            count += 1
            text += f'\n | rok{count} = {year} || pop{count} = {pop}'

        text += '\n| źródło = Narodowy Instytut Statystyczny<ref>{{Cytuj stronę | url = https://www.nsi.bg/nrnm/show9.php?sid=%s&ezik=en | tytuł = NATIONAL REGISTER OF POPULATED PLACES | opublikowany = Narodowy Instytut Statystyczny Bułgarii | język = en | data dostępu = 2021-12-28}}</ref>' % self.search_id
        text += '\n}}'
