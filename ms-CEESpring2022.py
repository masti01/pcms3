...
CEEtemplates = {'pl': 'Szablon:CEE Spring 2022', 'az': 'Şablon:Vikibahar 2022', 'ba': 'Ҡалып:Вики-яҙ 2022',
                'be': 'Шаблон:CEE Spring 2022', 'be-tarask': 'Шаблён:Артыкул ВікіВясны-2022',
                'bg': 'Шаблон:CEE Spring 2022', 'de': 'Vorlage:CEE Spring 2022', 'eo': 'Ŝablono:VikiPrintempo COE 2022',
                'el': 'Πρότυπο:CEE Spring 2022', 'et': 'Mall:CEE Spring 2022', 'hr': 'Predložak:CEE proljeće 2022.',
                'hu': 'Sablon:CEE Tavasz 2022', 'hy': 'Կաղապար:CEE Spring 2022', 'ka': 'თარგი:ვიკიგაზაფხული 2022',
                'lv': 'Veidne:CEE Spring 2022', 'lt': 'Šablonas:VRE 2022', 'mk': 'Шаблон:СИЕ Пролет 2022',
                'myk': 'Шаблон:СИЕ Пролет 2022', 'ro': 'Format:Wikimedia CEE Spring 2022',
                'ru': 'Шаблон:Вики-весна 2022', 'sr': 'Шаблон:ЦЕЕ пролеће 2022', 'tr': 'Şablon:Vikibahar 2022',
                'uk': 'Шаблон:CEE Spring 2022', 'en': 'Template:CEE Spring 2022'}
...
countryNames = {
    ...
    'ba': {'Албания': 'Albania', 'Австрия': 'Austria', 'Әзербайжан': 'Azerbaijan', 'Башҡортостан': 'Bashkortostan',
           ...
           'Босния': 'Bosnia and Herzegovina', 'Россия': 'Russia', 'Выру': 'Võro', },
    # be countries
    ...
    'bs': {'Albanija': 'Albania', 'Austrija': 'Austria', 'Azerbejdžan': 'Azerbaijan', 'Baškortostan': 'Bashkortostan',
           ...
           'Krimski Tatari': 'Crimean Tatars', 'Kipar':'Cyprus', 'Sjeverna Makedonija':'North Macedonia', 'Don':'Don',
    ...
    'mk': {'Албанија': 'Albania', 'Австрија': 'Austria', 'Азербејџан': 'Azerbaijan', 'Башкортостан': 'Bashkortostan',
           ...
           'Виру':'Võro', 'Роми':'Roma', 'Донски Регион': 'Don', },
    # ro countries
    ...
    'uk': {'Албанія': 'Albania', 'Австрія': 'Austria', 'Азербайджан': 'Azerbaijan', 'Башкортостан': 'Bashkortostan',
           ...
           'Боснія': 'Bosnia and Herzegovina', 'Боснія і Герцеговина‎': 'Bosnia and Herzegovina',
    ...
    'hr': {'Albaniji': 'Albania', 'Albanija': 'Albania', 'Austriji': 'Austria', 'Austrija': 'Austria',
           ...
           'Lužičkih Srba':'Sorbia', 'Romi':'Roma', 'Azerbejdžanu':'Azerbaijan', 'Cipru':'Cyprus', 'Esperanta':'Esperanto',
           'Krimu':'Crimean Tatars', 'Võru':'Võro', 'Lužici': 'Sorbia', 'Malte': 'Malta', },
    # sl countries
    'sl': {'Albanija': 'Albania', 'Armenija': 'Armenia', 'Avstrija': 'Austria', 'Azerbajdžan': 'Azerbaijan',
           ...
           'Ciper':'Cyprus', 'Võrumaa':'Võro', 'Lužiška srbija':'Sorbia', 'Lužiški Srbi':'Sorbia', 'Tatari': 'Crimean Tatars',
           },
    # mt countries
...
class BasicBot(
    ...
    def run(self):
        ...
        header = '{{TNT|Wikimedia CEE Spring 2022 navbar}}\n\n'
        header += '{{Wikimedia CEE Spring 2022/Statistics/Header}}\n\n'
        ...
        for l in aList.keys():
            for a in aList[l]:
                ...
                if user in self.womenAuthors.keys():
                    ...
                    self.womenAuthors[user]['list'].append((fam + ':' if fam != 'wikipedia' else '') + lang + ':' + a['title'])
                else:
                    self.womenAuthors[user] = {'count': 1, 'list': [(fam + ':' if fam != 'wikipedia' else '') + lang + ':' + a['title']]}
    ...
    def loadArticleList(self):
        ...
        if self.opt.reset:
            ...
            try:
                with open('masti/CEESpring2022.dat', 'rb') as datfile:
    ...
    def saveArticleList(self, artList):
        ...
        with open('masti/CEESpring2022.dat', 'wb') as f:
    ...
    def getArticleList(self):
        ...
        # p = pywikibot.Page( plwiki, "Szablon:CEE Spring 2022" )
        # while True:
        for p in self.generator:
            ...
            for i in self.genInterwiki(p):
                ...
                pywikibot.output(
                    'Getting references to %s Lang:%s Fam:%s' % (i.title(as_link=True, force_interwiki=True), lang, fam))
                if self.opt.test2:
                ...
                for p in i.getReferences(namespaces=1):
                    ...
                    if art.exists():
                        ...
                        if self.opt.testgetart:
                            pywikibot.output('getArticleList #%i/%i:%s:%s:%s' % (count, countlang, fam, lang, art.title()))
                        count += 1
            # break
    ...
    def getUpdater(self, art):
        ...
        # SpringStart = datetime.strptime("2022-03-20T12:00:00Z", "%Y-%m-%dT%H:%M:%SZ")
        ...
        for l in res.keys():
            ...
            for i in res[l]:
                ...
                if i['newarticle']:
                    ...
                    fam = i['family']
                    artLine = '\n|-\n| %i. || [[:%s:%s]] || %s || %s || ' % (
                        newarts, (fam + ':' if fam != 'wikipedia' else '') + i['lang'], i['title'], i['creator'], i['creationDate'])
                    cList = []
