...
class BasicBot(
    ...
    def run(self):
        ...
        count = 0
        marked = 0
        iw2check = ['frwiki', 'dewiki', 'eswiki', 'ptwiki', 'svwiki', 'nlwiki', 'ruwiki', 'arzwiki', 'plwiki', 'itwiki']
    ...
    def treat(self, page, iw2check):
        #
        try:
            wd = pywikibot.ItemPage.fromPage(page)
            wdcontent = wd.get()
            if self.opt.test:
                pywikibot.output(wdcontent['sitelinks'])
